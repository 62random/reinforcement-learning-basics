...
class QLearn:
    ...
    def __init__(self, alpha, gamma, action_space):
        self.qtable = {}
        self.alpha = alpha 
        self.gamma = gamma 
    ...
    def updateQValues(self, old_state, action, reward, new_state):
        # Preparing for bellman equation
        ...
        ### Action with best Q-value for the new state
        max_a_q_s1_a = max(new_state_qvalues.values())

        # Bellman Equation
        self.qtable[old_state][action] = q_s_a + self.alpha * (reward + self.gamma*max_a_q_s1_a - q_s_a)
